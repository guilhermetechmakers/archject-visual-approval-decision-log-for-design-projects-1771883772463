/**
 * 2FA Verify TOTP - Verify 6-digit code and enable 2FA
 * Body: { code: string }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!totpSecret) {
      return new Response(JSON.stringify({ message: '2FA setup not started. Run setup first.' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    console.error('two-fa-verify-totp error:', err)
